        ...
        while (!atEOF) {
                ...
                while (column++ <= columns) {
                        ...
                        while (lineNo++ < lines && !atEOF && !atFF) {
                                writestr ("(");
                                charNo = 0;
                                while (charNo++ < cols && (ch = getc (infile)) != EOF && !(ch == FF && doFFs) &&
                                       ch != '\n') {
                                        if (ch >= 32 && ch <= 127) {
                                                if (ch == '(' || ch == ')' || ch == '\\')
                                                        writestr ("\\");
...
void WriteRest ()
{
        ...
        sprintf (buf, "0000000000 65535 f %c", LINE_END);
        writestr (buf);
        for (i = 1; i <= curObj; i++) {
        ...
        while (i < argc) {
                if (*argv[i] != '-') { /* input filename */
                        ...
                        case 'f':
                                ...
                                strcat (font, ++argv[i]);
                                break;
                        case 'I': ISOEnc = 1; break;
                        case 'F': doFFs = 0; break;
                        case 's':
                        ...
                        case 'A':
                                ...
                                case '4':
                                        ...
                                        pageHeight = 842;
                                        break;
                                default: fprintf (stderr, "%s: ignoring unknown paper size: A%s\n", progname, argv[i]);
                                }
                                break;
                        case 'x':
                        ...
                        default: fprintf (stderr, "%s: ignoring invalid switch: -%s\n", progname, argv[i]);
                        }
                }
                i++;
        }
